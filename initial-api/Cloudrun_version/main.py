...
BASE_BUCKET_NAME = os.environ.get('BASE_BUCKET_NAME', 'watch_mail_trial_invoice')
...
def get_user_bucket_name(user_uid):
    """Generate user-specific bucket name"""
    # Use first 8 characters of UID for bucket naming (GCS bucket name limitations)
    # Convert to lowercase as GCS bucket names must be lowercase
    user_id_short = user_uid[:8].lower() if len(user_uid) >= 8 else user_uid.lower()
    return f"{BASE_BUCKET_NAME}_{user_id_short}"

def ensure_user_bucket_exists(user_uid):
    """Create user bucket if it doesn't exist"""
    bucket_name = get_user_bucket_name(user_uid)
    
    logger.info(f"🔍 Ensuring user bucket exists: {bucket_name}")
    ...
    try:
        ...
        if not bucket.exists():
            logger.info(f"📦 Creating user bucket: {bucket_name}")
            bucket = storage_client.create_bucket(bucket_name, location='asia-south1')
            logger.info(f"✅ User bucket created successfully: {bucket_name}")
        else:
            logger.info(f"📦 Using existing user bucket: {bucket_name}")
        
        return bucket_name
        
    except Exception as e:
        logger.error(f"❌ Failed to create/access user bucket {bucket_name}: {e}")
        logger.error(f"   User UID: {user_uid}")
        logger.error(f"   Bucket name format: {bucket_name}")
        # NO FALLBACK - user isolation is critical
        raise Exception(f"User bucket creation failed for {bucket_name}: {e}")
...
CORS(app, 
     resources={
         r"/*": {
             ...
             "supports_credentials": False  # Disable credentials for now to fix CORS
         }
     })
...
def upload_file():
    ...
    # Extract client organization context from form data
    client_organization_context = request.form.get('organization_context')
    client_organization_id = request.form.get('client_organization_id')
    client_organization_name = request.form.get('client_organization_name')
    
    if client_organization_context:
        logger.info(f"🏢 Client organization context provided: {client_organization_name} (ID: {client_organization_id})")
        # Add client org context to user_context for downstream processing
        user_context['client_organization_context'] = client_organization_context
        user_context['client_organization_id'] = client_organization_id
        user_context['client_organization_name'] = client_organization_name
    else:
        logger.info("📋 No client organization context provided - using legacy processing")
    
    try:
        # Ensure user bucket exists
        user_bucket_name = ensure_user_bucket_exists(user_context['uid'])
        
        # Generate production-ready job ID with customer tracking
        import hashlib
        import secrets
        
        # Generate unique path with timestamp (no user folder needed since bucket is user-isolated)
        timestamp = datetime.utcnow().strftime('%Y/%m/%d/%H-%M-%S')
        filename = f"raw/{timestamp}_{file.filename}"
        
        logger.info(f"Processing upload for file: {filename} in bucket: {user_bucket_name}")
        
        # Get user bucket and create a blob
        bucket = storage_client.bucket(user_bucket_name)
        blob = bucket.blob(filename)
        
        # Upload the file
        blob.upload_from_file(file)
        logger.info(f"Successfully uploaded file to {user_bucket_name}/{filename}")
        ...
        # Call orchestrator to trigger pipeline
        # Architecture: initial-api → orchestrator → data-extractor-v2 → enrichment-worker → field-standardizer-v2
        try:
            auth_header = request.headers.get('Authorization')
            if not auth_header:
                logger.error("No authorization header for processing call")
                return jsonify({"error": "Authentication required for processing"}), 401
            
            # Extract token from header
            auth_token = auth_header.split(' ')[1] if auth_header.startswith('Bearer ') else auth_header
            
            # Service URL for orchestrator (watch-mail-trial project)
            ORCHESTRATOR_URL = "https://orchestrator-service-812016027146.asia-south1.run.app"
            
            # Create job document in Firestore first
            tenant_id = user_context['uid']
            gcs_path = f"gs://{user_bucket_name}/{filename}"
            ...
            firestore_client.collection('tenants').document(tenant_id).collection('jobs').document(job_id).set(job_doc)
            logger.info(f"📝 Created job document for {job_id}")
            
            # Also write to users/{uid}/invoice_results for frontend visibility
            # This allows frontend to show "processing" status immediately
            user_result_doc = {
                'id': job_id,
                'status': 'processing',
                'original_filename': file.filename,
            ...
            firestore_client.collection('users').document(tenant_id).collection('invoice_results').document(job_id).set(user_result_doc)
            logger.info(f"📝 Created user result for frontend: users/{tenant_id}/invoice_results/{job_id}")
            
            # Prepare payload for orchestrator
            orchestrator_payload = {
                'gcs_input_path': gcs_path,
                'client_id': user_context.get('client_organization_id')
            }
            
            logger.info(f"🎼 Calling orchestrator for job: {job_id}")
            
            # Call orchestrator asynchronously (don't wait for response)
            import requests
            import threading
            def call_orchestrator():
                try:
                    response = requests.post(
                        f"{ORCHESTRATOR_URL}/jobs/{job_id}/trigger-extraction",
                    ...
                    if response.ok:
                        logger.info(f"✅ Orchestrator triggered for job: {job_id}")
                    else:
                        logger.error(f"❌ Orchestrator failed for job {job_id}: {response.status_code}")
                        # Update job status to FAILED
                        firestore_client.collection('tenants').document(tenant_id).collection('jobs').document(job_id).update({
                            'status': 'FAILED',
                            'error': f"Orchestrator failed: {response.status_code}",
                            'updated_at': datetime.utcnow().isoformat()
                        })
                except Exception as e:
                    logger.error(f"❌ Orchestrator call failed for job {job_id}: {e}")
                    firestore_client.collection('tenants').document(tenant_id).collection('jobs').document(job_id).update({
                        'status': 'FAILED',
                        'error': str(e),
                        'updated_at': datetime.utcnow().isoformat()
            ...
            threading.Thread(target=call_orchestrator, daemon=True).start()
            
            processing_mode = "orchestrated_pipeline"
            logger.info(f"✅ File uploaded and orchestrator triggered")
            
        except Exception as process_error:
            logger.error(f"Failed to trigger orchestrator: {process_error}")
            # Don't fail the upload, just log the error
            logger.warning(f"Upload succeeded but orchestrator call failed for job_id: {job_id}")
        
        # Return successful upload response
        logger.info(f"File uploaded successfully. Job ID: {job_id}, Bucket path: {user_bucket_name}/{filename}")
        ...
        return jsonify({
            ...
            "gcsPath": f"gs://{user_bucket_name}/{filename}",
            "bucket_path": f"{user_bucket_name}/{filename}",
            "orchestrator_triggered": True,
            "processing_mode": processing_mode,
            "selected_processing": processing_mode
        }), 200
...
def health_check():
    ...
    return jsonify({
        ...
        "queue_system": "user_isolated_queues",
        "base_queue_name": cloud_tasks_manager.base_queue_name
    }), 200
